...
d=200
ges={"l": startv, "m": startv, "r": startv}
#Hauptprogramm
break_=False
run_=True
while run_==True:
    while break_==False:
        ...
        mm.run(int(ges["m"]))
        rr.run(int(ges["r"]))
        ev3.screen.print(str(time_ms()-nn))
        wait(10)
        ev3.screen.clear()
        if auto.pressed()==True:
            ...
            mm.hold()
            rr.hold()
        while int(time_ms()-nn)%5!=0:
            pass
        d=int(sqrt(int(time_ms()-nn)))+200
        ges["l"]=randint(1, int(d))
        ges["m"]=randint(d/2, 2*int(d))
        ges["r"]=(int(d)-(int(ges["l"])+int(ges["r"]))/3)*3
